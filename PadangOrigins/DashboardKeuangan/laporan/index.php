...
$trxData = safeLoad($fileTrx);
$historiLaporan = safeLoad($fileLaporan);
$rekeningList = safeLoad($fileRekening);
...
<head>
    ...
    <style>
        ...
        .balance-card { background: white; padding: 1.2rem; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); border: 1px solid #f1f1f1; display: flex; flex-direction: column; height: 100%; }
        ...
        .bg-purple { background: #9b59b6; } .bg-dark { background: #34495e; } .bg-blue { background: #3498db; } .bg-orange { background: #e67e22; } .bg-red { background: #ef4444; }
        ...
        .btn-ops { background: #f97316; color: white; padding: 8px 15px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600; display: flex; align-items: center; gap: 5px; box-shadow: 0 4px 10px rgba(249, 115, 22, 0.3); }
        ...
        .tipe-masuk { background: #dcfce7; color: #166534; } .tipe-keluar { background: #fee2e2; color: #991b1b; } .tipe-transfer { background: #e3f2fd; color: #3498db; }
        .alert-status { text-align: center; padding: 10px; margin-bottom: 20px; border-radius: 8px; font-weight: bold; background: #dcfce7; color: #166534; }
    </style>
</head>
<body>
    <header class="navbar">
    ...
    <main class="container">
        <?php if(isset($_GET['status'])): ?>
            <div class="alert-status">
                <?php echo $_GET['status']=='success_ops' ? "✅ Pengeluaran Operasional Berhasil Dicatat!" : "✅ Transaksi Berhasil!"; ?>
            </div>
        <?php endif; ?>
        ...
        <div class="widgets-container">
            <?php if (!empty($myReports)): ?>
                <?php foreach ($myReports as $idx => $rep): 
                    $fileTarget = $dataDir . $rep['source'];
                    $data = file_exists($fileTarget) ? json_decode(file_get_contents($fileTarget), true) : []; ?>
    ...
    <div id="modalKasOps" class="modal-overlay">
        <div class="modal-box">
            <div style="display:flex; justify-content:space-between; margin-bottom:1.5rem;">
                <h2 style="margin:0; color:#f97316;">⚡ Ambil Kas Operasional</h2>
                <span onclick="tutupModal('modalKasOps')" style="cursor:pointer; font-size:1.5rem;">×</span>
            </div>
            <form method="POST" action="proses_laporan.php">
                <input type="hidden" name="action" value="simpan_ops">
                <div class="form-group">
                    <label style="display:block; font-weight:bold; margin-bottom:5px;">Alasan Pengeluaran</label>
                    <input type="text" name="alasan" class="form-control" required placeholder="Beli Bensin, Plastik, dll">
                </div>
                <div class="form-group">
                    <label style="display:block; font-weight:bold; margin-bottom:5px;">Jumlah (Rp)</label>
                    <input type="number" name="jumlah" class="form-control" required min="1">
                </div>
                <button type="submit" style="width:100%; background:#f97316; color:white; padding:10px; border:none; border-radius:8px; font-weight:bold; cursor:pointer;">Simpan</button>
            </form>
        </div>
    </div>
    ...
    <div id="modalTransfer" class="modal-overlay">
        <div class="modal-box">
            ...
            <form method="POST" action="proses_laporan.php">
                ...
                <label>Dari</label><select name="sumber" id="tf_sumber" class="form-control"><option value="kas_laci">Kas Laci</option><option value="kas_ops">Kas Ops</option><optgroup label="Bank"><?php foreach($rekeningList as $r) echo "<option value='{$r['nama_bank']}'>{$r['nama_bank']}</option>"; ?></optgroup></select>
                <label>Ke</label><select name="tujuan" class="form-control"><option value="kas_ops">Kas Ops</option><option value="kas_laci">Kas Laci</option><optgroup label="Bank"><?php foreach($rekeningList as $r) echo "<option value='{$r['nama_bank']}'>{$r['nama_bank']}</option>"; ?></optgroup></select>
    ...
    <div id="modalPrive" class="modal-overlay">
        <div class="modal-box">
            <div style="display:flex; justify-content:space-between; margin-bottom:1rem;"><h2>Prive Owner</h2><span onclick="document.getElementById('modalPrive').style.display='none'" style="cursor:pointer;">×</span></div>
            <form method="POST" action="proses_laporan.php">
                <input type="hidden" name="action" value="tarik_prive">
                <label>Sumber</label><select name="sumber" class="form-control"><option value="kas_laci">Kas Laci</option><optgroup label="Bank"><?php foreach($rekeningList as $r) echo "<option value='{$r['nama_bank']}'>{$r['nama_bank']}</option>"; ?></optgroup></select>
                <label>Jumlah</label><input type="number" name="jumlah" class="form-control" required>
                <label>Catatan</label><input type="text" name="catatan" class="form-control" required>
                <button type="submit" style="width:100%; background:#34495e; color:white; padding:10px; border:none; border-radius:8px;">Tarik Dana</button>
            </form>
        </div>
    </div>
    ...
    <div id="modalTutup" class="modal-overlay">
        <div class="modal-box">
            ...
            <p>Saldo akan disimpan. Transaksi harian akan di-reset.</p>
            <form method="POST" action="proses_laporan.php">
