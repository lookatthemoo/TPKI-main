...
<head>
    ...
    <style>
        ...
        /* HEADER KARTU (PERBAIKAN AGAR TIDAK NUMPUK) */
        .emp-header { 
            display: flex; 
            ...
            position: relative; /* Penting untuk tombol absolute jika diperlukan */
        }
        
        .emp-info { flex: 1; padding-right: 10px; } /* Beri ruang agar teks tidak nabrak */
        ...
        /* CONTAINER POJOK KANAN (Status + Delete) */
        .header-actions {
            display: flex;
        ...
        .btn-gaji { background: #2563eb; color: white; }
        ...
        /* CSS MODAL DELETE */
        .modal-delete-content { text-align: center; }
        .btn-confirm-delete { background: #ef4444; color: white; padding: 10px 20px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; margin-right: 10px; }
        .btn-cancel { background: #e2e8f0; color: #333; padding: 10px 20px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; }
...
<body>
    ...
    <main class="container">
        ...
        <?php if(isset($_GET['status'])): ?>
            <div style="background:#dcfce7; color:#166534; padding:12px; border-radius:8px; margin-bottom:20px; text-align:center; font-weight:600; border:1px solid #bbf7d0;">
                <?php 
                    if($_GET['status'] == 'bonus_ready') echo "✅ Bonus berhasil dikirim ke Dompet Karyawan!";
                    elseif($_GET['status'] == 'salary_ready') echo "✅ Gaji Bulanan berhasil dikirim!";
                    elseif($_GET['status'] == 'deleted') echo "🗑️ Karyawan berhasil dipecat (Data Dihapus)!";
                ?>
            </div>
        <?php endif; ?>
            ...
            <div class="emp-card blue-top">
                <div class="emp-header">
                    ...
                    <div class="header-actions">
                        ...
                        <button type="button" class="btn-delete-small" onclick="confirmDelete('<?php echo $k['id']; ?>', '<?php echo htmlspecialchars($k['nama']); ?>')" title="Pecat Karyawan">🗑️</button>
                    </div>
                </div>
                ...
                <div style="margin-top:auto;"> 
                    <form action="proses_karyawan.php" method="POST" onsubmit="return confirm('Kirim Gaji Bulanan?');">
                        <input type="hidden" name="action" value="pay_salary">
                        <input type="hidden" name="id" value="<?php echo $k['id']; ?>">
                        <input type="hidden" name="nama" value="<?php echo $k['nama']; ?>">
                        <input type="hidden" name="amount" value="<?php echo $k['gaji_pokok']; ?>">
                        <button type="submit" class="btn-action btn-gaji">💸 Kirim Gaji Full</button>
                    </form>
                </div>
            </div>
            <?php endforeach; ?>
        ...
        <div id="view-karyawan" class="view-section <?php echo $activeTab === 'karyawan' ? 'active' : ''; ?>">
            ...
            <div class="emp-card orange-top">
                ...
                <div class="stats-row">
                    <div class="stat-item">
                        <span class="stat-val"><?php echo $k['hadir']; ?></span>
                        <span class="stat-lbl">Hadir</span>
                    </div>
                    <div class="stat-item">
                        <span class="stat-val"><?php echo $k['izin']; ?></span>
                ...
                <form action="proses_karyawan.php" method="POST" class="control-panel">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="number" name="hadir" value="<?php echo $k['hadir']; ?>" class="form-input" title="Jumlah Hadir">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="number" name="tambah_bonus" placeholder="Nominal Rp.." class="form-input">
                ...
                <div class="money-box">
                    ...
                    <?php if(($k['bonus_pending'] ?? 0) > 0): ?>
                        <form action="proses_karyawan.php" method="POST">
                            <input type="hidden" name="action" value="pay_bonus">
                            <input type="hidden" name="id" value="<?php echo $k['id']; ?>">
                            <input type="hidden" name="amount" value="<?php echo $k['bonus_pending']; ?>">
                            <button type="submit" class="btn-action btn-cair">⚡ Kirim ke Dompet</button>
                        </form>
                    <?php else: ?>
    ...
    <button class="float-add" onclick="openModal('modalAdd')">+</button>

    <div id="modalAdd" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header"><h2>Tambah Karyawan</h2><span class="close-modal" onclick="closeModal('modalAdd')">&times;</span></div>
            <form action="proses_karyawan.php" method="POST">
                <input type="hidden" name="action" value="add_employee">
                <div class="modal-form-group"><label>Nama Lengkap</label><input type="text" name="nama" required></div>
                <div class="modal-form-group"><label>Posisi</label><input type="text" name="posisi" required></div>
                <div class="modal-form-group"><label>Gaji Pokok (Rp)</label><input type="number" name="gaji" required></div>
                <button type="submit" class="btn-submit-wd" style="background:#2563eb;">Simpan</button>
            </form>
        </div>
    </div>
    ...
    <div id="modalDelete" class="modal-overlay">
        <div class="modal-content modal-delete-content">
            ...
            <p style="margin-bottom:20px; color:#555;">Apakah Anda yakin ingin menghapus data <strong id="deleteTargetName">...</strong>?<br>Tindakan ini tidak dapat dibatalkan.</p>
            ...
            <form action="proses_karyawan.php" method="POST">
                ...
                <button type="button" class="btn-cancel" onclick="closeModal('modalDelete')">Tidak</button>
            </form>
        </div>
    </div>
